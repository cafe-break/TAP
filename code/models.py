...
class BaseClassifier(nn.Module):
    ...
    def forward(self, spt_embedding_i, loss_type, scale=0.1, node_degree=None):
        if loss_type == 'crossentropy':
            ...
            if node_degree is None:
                ...
                d_max = torch.max(node_degree)
                wf = (node_degree/d_max) * F.linear(F.normalize(spt_embedding_i, p=2, dim=1), F.normalize(self.fc.weight, p=2, dim=1))
        return wf
