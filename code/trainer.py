...
def test_base(encoder, features, curr_adj, base_val_set, cls_prototypes, labels,
    ...
    labels_val = labels_val.cpu().detach()
    acc_val = accuracy(val_output, labels_val)
    print('Session 0: all_test acc:{:.4f}, base_test_acc: {:.4f}, seen_test_acc: {:.4f}, novel_test_acc: {}'.format(
        acc_val, acc_val, 0., 0.))
...
def test_incremental(novel_encoder, features, curr_adj, base_id_query, novel_id_query, seen_id_query,
    ...
    all_f1_test = f1(pairwise_distance_all, labels_all_query)

    base_acc_test = accuracy(pairwise_distance_all[:len(base_id_query), :], labels_all_query[:len(base_id_query)])
    base_f1_test = f1(pairwise_distance_all[:len(base_id_query), :], labels_all_query[:len(base_id_query)])

    seen_acc_test = accuracy(pairwise_distance_all[len(base_id_query):, :], labels_all_query[len(base_id_query):])
    seen_f1_test = f1(pairwise_distance_all[len(base_id_query):, :], labels_all_query[len(base_id_query):])
    novel_acc_test = []

    for j in range(len(n_novel_list) - 1):
        n_start = len(base_id_query) + n_novel_list[j]
        n_end = len(base_id_query) + n_novel_list[j + 1]
        novel_acc_test.append(
            accuracy(pairwise_distance_all[n_start:n_end, :], labels_all_query[n_start:n_end:]).item())

    return all_acc_test, all_f1_test, base_acc_test, base_f1_test, seen_acc_test, seen_f1_test, embeddings, labels, novel_acc_test
